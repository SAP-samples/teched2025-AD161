...
In VS Code, open a terminal and create the xflights project with
```sh
cds init xflights
```
...
Even though the app is still empty, we already now start it.
...
After completing these steps you will have defined the essential entities of the _xflights_ app.

The first part of the new app is the domain model.
...
__TODO__ add picture


As soon as you save the file _schema.cds_, the still running cds watch
...
In folder _xflights/srv_, create a file _fiori-service.cds_ and fill it with this content:
```cds
...
The entities are exposed via OData, but still empty.
...
Go back to the index page [localhost:4005](http://localhost:4005/) and click
on any of the files to see the new content.
...
Wait until `cds watch` has picked up the changes, then go back to the
...
In folder _xflights/db_, add a file _labels.cds_ and copy the content of
...
The values of the annotations are `i18n` keys. We need to also get the respective texts into our app.
Copy the folder [assets/ex1/_i18n](../../assets/ex1/_i18n) into folder _xflights_.

The result should look like this:
<br>![](/exercises/ex1/images/01_06_0010.png)

Go back to the browser window with the Fiori preview and refresh.
Notice the change in the column labels.


## Exercise 1.7 - Add API service

After completing these steps you will have an additional service in
that acts as an API to retrieve the master data from the _xflights_ app.

In folder _xflights/srv_, add a file _data-service.cds_ with this content:
```cds
...
Assuming that `cds watch` is still running, you can go to the index page in the browser
and see the new service being presented via these protocols.

Click for example the [Flights](http://localhost:4005/odata/v4/data/Flights) link in
...
As the auto-exposure mechanism of the compiler doesn't yet seemlessly work
